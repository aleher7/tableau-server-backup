...
def subir_github(directorio_base, config, token=None, mensaje=None):
    ...
    try:
        ...
        # commits nuevos del remoto y los combina con los locales -- así, si
        # el remoto avanzó mientras tanto, el push de después no se rechaza.
        # --no-edit evita que se abra un editor de texto pidiendo el mensaje
        # del commit de fusión (usa uno automático).
        logger.info("[GIT] git pull (sincronizando con el remoto antes de subir)")
        codigo_pull, salida_pull = ejecutar_git(['git', 'pull', '--no-edit', url_con_token, 'main'])
        if codigo_pull != 0:
