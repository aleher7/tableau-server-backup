...
if faltantes:
    print(f"   ❌ Faltan claves: {faltantes}")
    exit()
print("   ✅ OK\n")
...
if resp.status_code != 201:
    print(f"   ❌ Código {resp.status_code}: {resp.text[:300]}")
    exit()
token = resp.json()['token']
print("   ✅ OK (token válido ~1h)\n")
...
if resp2.status_code != 200:
    print(f"   ❌ Código {resp2.status_code}: {resp2.text[:300]}")
    exit()

print(f"   ✅ OK - contenido de {owner}/{repo}:")
