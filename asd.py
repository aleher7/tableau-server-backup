...
def descargar_workbook(server, workbook_luid, ruta_destino):
    ...
    ⚠️ CORRECCIÓN CRÍTICA (2025):
    ────────────────────────────
    tableauserverclient.download() tiene un comportamiento especial:
    
    PROBLEMA:
    - Si pasas filepath="/path/Dashboard.twbx" lo interpreta como un DIRECTORIO
    - Crea la carpeta "/path/Dashboard.twbx/" 
    - Descarga el archivo DENTRO de esa carpeta
    - Resultado: Carpeta innecesaria con nombre .twbx
    
    SOLUCIÓN IMPLEMENTADA:
    - Descargar sin extensión: "/path/Dashboard"
    - TSC crea: "/path/Dashboard/" (carpeta normal)
    - Movemos el .twbx a su ubicación final
    - Eliminamos la carpeta temporal
    
    Args:
        server: Conexión a Tableau
        workbook_luid: ID del workbook en Tableau
        ruta_destino: Ruta local donde guardar (con .twbx)
    ...
    try:
        ...
        # ✅ PASO 1: Descargar SIN extensión .twbx
        ...
        # ✅ PASO 2: Procesar el archivo descargado
        ...
        if carpeta_descargada.is_dir():
            ...
            if archivos_twbx:
                ...
                # ✅ PASO 3: Limpiar la carpeta temporal
                try:
