...
def main():
    ...
    # --- Prueba 6: marcar_subido_en_oracle() genera el SQL correcto ---
    # No se ejecuta de verdad (no hay Oracle aqui): solo se comprueba que
    # el fichero temporal se construye con la sintaxis correcta.
    print("\n=== Prueba 6: marcar_subido_en_oracle() (solo construccion del SQL) ===")
    config_falsa = {'sqlplus_marcar_comando': 'echo'}  # "echo" para no fallar al intentar ejecutar
    pares = [('luid-aaa-111', '5'), ('luid-bbb-222', '2')]
    dw.marcar_subido_en_oracle(config_falsa, pares)
    print("(revisa que no haya lanzado ninguna excepcion de Python al construir el UPDATE)")
    print("OK\n")
