...
def leer_archivo_datos(ruta_archivo):
    ...
    try:
        ...
        if extension == '.dsv':
            ...
            logger.warning("[AVISO] Extension no reconocida, intentando como CSV con coma")
            df = pd.read_csv(ruta_archivo, sep=',', quotechar='"', doublequote=True, skipinitialspace=True, on_bad_lines='skip')
        ...
        col_luid = buscar_columna(df, ['WORKBOOK_LUID', 'LUID'])  # Buscar LUID PRIMERO
        col_nombre = buscar_columna(df, ['WORKBOOK'])  # Luego WORKBOOK
