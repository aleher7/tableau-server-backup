...
def sincronizar_con_remoto(directorio_base, config):
    ...
    try:
        ...
        # URL LIMPIA, sin credenciales -- el token va por cabecera HTTP
        # (ver construir_credencial_git), nunca aparece en ningún mensaje.
        url = f"https://cantabrialabs.ghe.com/{owner}/{repo}.git"
        extra_header = construir_credencial_git(token)

        # Por si quedó algún merge a medias de una ejecución anterior
        ejecutar_git(['git', 'merge', '--abort'], secretos=[token])

        logger.info("[GIT] git fetch")
        codigo, salida = ejecutar_git(
            ['git', '-c', extra_header, 'fetch', url, 'main'], secretos=[token]
        )
        if codigo != 0:
        ...
        codigo, salida = ejecutar_git(['git', 'reset', '--hard', 'FETCH_HEAD'], secretos=[token])
        if codigo != 0:
...
def limpiar_directorio(directorio_base):
    ...
    if ruta.exists():
        logger.info("[LIMPIANDO] Vaciando contenido de: %s (conservando .git y .gitattributes)", directorio_base)
        # Archivos/carpetas que NUNCA se deben borrar, aunque estén dentro
        # del directorio de descarga: ".git" (config del repo) y
        # ".gitattributes" (configuración de Git LFS -- si se borrara aquí,
        # la siguiente descarga volvería a subir los .twbx grandes como
        # blobs normales en vez de por LFS, repitiendo el error de "exceeds
...
def descargar_workbook(server, workbook_luid, ruta_destino):
    ...
    try:
        ...
        if carpeta_descargada.is_dir():
            # ...y luego se saca el archivo de dentro de esa carpeta y se
            # mueve al destino final. Puede ser ".twbx" (workbook con datos
            # empaquetados/extracto) o ".twb" (workbook SIN datos
            # empaquetados, conectado directo a una fuente externa) --
            # ambos son formatos válidos, así que se aceptan los dos.
            archivos = list(carpeta_descargada.glob('*.twbx')) + list(carpeta_descargada.glob('*.twb'))

            if archivos:
                archivo_real = archivos[0]
                # Ajusta la extensión del destino a la real (si Tableau
                # devolvió .twb en vez de .twbx, el archivo final también
                # se guarda como .twb -- si se forzara a .twbx igualmente,
                # el archivo quedaría con la extensión "equivocada" y
                # Tableau/Windows no lo reconocerían bien al abrirlo).
                ruta_destino_real = ruta_destino.with_suffix(archivo_real.suffix)
                shutil.move(str(archivo_real), str(ruta_destino_real))
                try:
                    shutil.rmtree(carpeta_descargada)  # Limpia la carpeta temporal, ya vacía
                    logger.info("[OK] Descargado: %s", ruta_destino_real.name)
                except:
                    # No es grave si falla el borrado de la carpeta temporal:
                    # el archivo ya se movió correctamente a su sitio.
                    pass
                return True
            else:
                logger.error("[ERROR] No se encontró .twbx ni .twb")
                return False
        else:
            # Algunos workbooks se descargan directamente como archivo
            # (sin crear carpeta intermedia), dependiendo de la versión de
            # TSC. Se comprueba tanto la ruta ".twbx" esperada como su
            # variante ".twb", por si Tableau lo entregó sin empaquetar.
            ruta_alternativa = ruta_destino.with_suffix('.twb')
            if ruta_destino.exists():
                logger.info("[OK] Descargado: %s", ruta_destino.name)
                return True
            elif ruta_alternativa.exists():
                logger.info("[OK] Descargado: %s", ruta_alternativa.name)
                return True
            else:
...
def construir_credencial_git(token, url_git="https://cantabrialabs.ghe.com"):
    """
    Prepara la autenticación de git SIN meter el token en la URL.

    Antes: https://x-access-token:{token}@servidor/repo.git
      -> problema: git muestra esa URL completa (con el token) en sus
         propios mensajes ("warning: current Git remote contains
         credentials", "Locking support detected on..."), así que el
         token queda expuesto en la consola aunque nuestro script no lo
         imprima directamente.

    Ahora: se manda como cabecera HTTP "Authorization: Basic ...", vía
    "git -c http.extraHeader=...". Esa cabecera viaja por la conexión
    pero NUNCA aparece en los mensajes que imprime git, porque no forma
    parte de la URL ni de ningún dato que git "muestre".

    IMPORTANTE (descubierto tras el error "Authentication required" al
    subir archivos por LFS): un "http.extraHeader" SIN restringir se
    envía a CUALQUIER servidor con el que hable git durante ese comando
    -- incluido el almacén de objetos de LFS, que vive en un DOMINIO
    DISTINTO (objects-origin.cantabrialabs.ghe.com) y usa sus propias
    URLs firmadas. Nuestra cabecera "de más" choca con esa autenticación
    propia y el push de LFS falla. La solución es usar la forma
    "http.<URL>.extraHeader", que restringe la cabecera a que SOLO se
    envíe a ese dominio concreto (el de git), dejando que LFS gestione
    su propia autenticación con el almacén de objetos sin interferencias.

    Devuelve: el argumento -c para pasarle a git, ya restringido por URL.
    """
    import base64
    credencial_b64 = base64.b64encode(f"x-access-token:{token}".encode()).decode()
    extra_header = f"http.{url_git}.extraHeader=Authorization: Basic {credencial_b64}"
    return extra_header


def redactar_secretos(texto, *secretos):
    """
    Sustituye cualquier secreto (token, etc.) que aparezca dentro de un
    texto por '***' antes de imprimirlo/loguearlo. Red de seguridad
    adicional: aunque ya no deberíamos imprimir tokens en ningún sitio,
    esto evita que un mensaje inesperado de git (o un cambio futuro en
    el código) filtre un secreto por descuido.
    """
...
def ejecutar_git(comando, secretos=None):
    """
    ...
    Parámetro 'secretos': lista de strings (tokens, contraseñas...) que se
    deben OCULTAR (sustituir por ***) tanto en la línea de comando que se
    imprime como en cada línea de salida de git, antes de mostrarla o
    guardarla en el log. Esto es imprescindible porque, aunque ya no
    metemos el token en la URL, sigue siendo una red de seguridad por si
    algún mensaje de git lo mencionara de todos modos.

    Devuelve (codigo_salida, texto_completo_de_la_salida_ya_redactada).
    """
    secretos = secretos or []

    comando_para_log = [redactar_secretos(str(c), *secretos) for c in comando]
    logger.info("       $ %s", " ".join(comando_para_log))
    ...
    for linea in proceso.stdout:
        linea = linea.rstrip()
        if linea:
            linea_segura = redactar_secretos(linea, *secretos)
            logger.info("       git> %s", linea_segura)  # se imprime AL MOMENTO, ya redactada
            lineas.append(linea_segura)
...
def subir_github(directorio_base, config, token=None, mensaje=None):
    ...
    try:
        ...
        # --renormalize: fuerza a git a re-aplicar el .gitattributes (LFS)
        # aunque ya exista en el repositorio un objeto con el mismo
        # contenido creado ANTES de activar LFS -- evita que un .twbx se
        # "cuele" como blob normal por reutilizar un objeto antiguo.
        # NOTA: NO usar --renormalize aquí -- implica -u/--update, que solo
        # afecta a archivos YA rastreados y NUNCA añade archivos nuevos.
        # Como la mayoría de los .twbx son "nuevos" en cada ejecución
        # (descarga fresca cada vez), --renormalize los ignoraba en
        # silencio y no se comiteaba nada.
        #
        # En su lugar: "git rm --cached" saca del índice cualquier rastro
        # de una versión anterior sin pasar por LFS, y "git add" lo vuelve
        # a añadir de cero, forzando el filtro de LFS.
        ejecutar_git(['git', 'rm', '-r', '--cached', '--ignore-unmatch', '.'])
        ...
        # Aquí SÍ hace falta el token: se pasa por CABECERA HTTP (nunca en
        # la URL), así que ni git ni nuestros propios logs pueden llegar a
        # mostrarlo por accidente (ver construir_credencial_git).
        owner = config['github_owner']
        repo = config['github_repo_name']
        ...
        ejecutar_git(['git', 'merge', '--abort'], secretos=[token])
        codigo_pull, salida_pull = ejecutar_git(
            ['git', '-c', extra_header, 'pull', '--no-edit', '-X', 'ours', url, 'main'],
            secretos=[token]
        )
        if codigo_pull != 0:
        ...
        codigo, salida = ejecutar_git(['git', '-c', extra_header, 'push', url, 'main'], secretos=[token])
...
def main():
    ...
    if not args.sin_github and config.get('github_enabled', True):
        ...
        # Refrescar la referencia local "origin/main" -- un "git push a
        # URL directa" sube los datos pero NO actualiza esta referencia
        # automáticamente (eso solo pasa al usar el remoto "origin" por
        # nombre). Sin esto, un "git status" posterior mostraría commits
        # "por delante" aunque ya estén todos subidos de verdad.
        token_final = obtener_installation_token(
            config['github_client_id'], config['github_installation_id'], config['github_private_key_path']
        )
        if token_final:
            owner = config['github_owner']
            repo = config['github_repo_name']
            url_final = f"https://cantabrialabs.ghe.com/{owner}/{repo}.git"
            extra_header_final = construir_credencial_git(token_final)
            os.chdir(directorio_base)
            ejecutar_git(['git', '-c', extra_header_final, 'fetch', url_final, 'main'], secretos=[token_final])
            ejecutar_git(['git', 'update-ref', 'refs/remotes/origin/main', 'FETCH_HEAD'], secretos=[token_final])
    else:
